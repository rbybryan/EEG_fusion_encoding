...
for f, fmaps in enumerate(fmaps_list):
	...
	if args.layers == 'all':
		if f == 0:
			print(feats.shape)
			feats_all = np.empty((len(fmaps_list), len(feats)), dtype=np.float32)
		feats_all[f, :] = feats
		
	# except Exception as e:
    #     	print(f"Error loading {fmaps}: {e}")
if args.layers == 'all':
...
import pickle
# Save scaler
with open(os.path.join(save_dir,args.dnn +'_' + str(args.pretrained) + '_scaler.pkl'), 'wb') as f:
    pickle.dump(scaler, f, protocol=4)

# Save PCA
with open(os.path.join(save_dir,args.dnn +'_' + str(args.pretrained) + '_pca.pkl'), 'wb') as f:
