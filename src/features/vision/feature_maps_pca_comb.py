"""Reduce DNN feature map dimensionality with PCA, combined with text features.
...
parser.add_argument('--dnn', default='cornet_s', type=str,
                    help="Used DNN among 'alexnet', 'resnet50', 'cornet_s', "
                         "'moco'.")
parser.add_argument('--pretrained', default=True, type=bool,
                    help='If True use pretrained network feature maps.')
...
data_dict = np.load(op.join(args.project_dir, 'gpt4_features',
                            text_embedding_file), allow_pickle=True).item()
...
for l, dnn_layer in enumerate(layer_names):
    ...
    fmaps_train[dnn_layer] = np.concatenate(
        [fmaps_train[dnn_layer], text_features_train_long], axis=-1)
    print(fmaps_train[dnn_layer].shape)
...
save_dir = os.path.join(args.project_dir, 'dnn_feature_maps',
                        'pca_feature_maps',
                        args.dnn + str(args.n_components),
                        'pretrained-' + str(args.pretrained),
                        'layers-' + args.layers)
...
for l, dnn_layer in enumerate(layer_names):
    ...
    np.save(os.path.join(save_dir, 'explained_variance_%d.npy' % l),
            explained_variances)
...
for l, dnn_layer in enumerate(layer_names):
    ...
    fmaps_test[dnn_layer] = np.concatenate(
        [fmaps_test[dnn_layer], text_features_test_long], axis=-1)
    print(fmaps_test[dnn_layer].shape)
