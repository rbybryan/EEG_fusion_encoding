"""Utility functions for preprocessing the THINGS EEG2 data.

This module provides helpers to epoch the raw EEG recordings, z-score the
epoched data session by session, and merge and save the preprocessed test and
training partitions.
"""


def epoching(args, data_part, seed):
    """Convert the EEG data to MNE raw format and preprocess it.

    This function first converts the EEG data to MNE raw format, and performs
    channel selection, filtering, epoching, current source density transform,
    frequency downsampling and baseline correction. Then, it sorts the EEG data
    of each session based on the image conditions.
    ...
    for s in range(args.n_ses):
        ...
        eeg_dir = os.path.join('raw_data', 'sub-' + format(args.sub, '02'),
                               'ses-' + format(s + 1, '02'),
        ...
        if args.csd == 1:
            ...
            eeg_dir = os.path.join(
                args.things_eeg_2_dir, 'source_data',
                'sub-' + format(args.sub, '02'),
                'ses-' + format(s + 1, '02'), 'eeg',
                'sub-' + format(args.sub, '02') + '_ses-' +
                format(s + 1, '02') + '_task-test_eeg.vhdr')
...
def save_prepr(args, zscored_test, zscored_train, img_conditions_train,
               ch_names, times, seed):
    """Merge, shuffle and save the preprocessed EEG data.
    ...
    del merged_test
    # Saving directories
    save_dir = os.path.join(args.project_dir, 'preprocessed_eeg_data_v1')
