"""
Multi-target Ridge regression for batch training across all EEG channels and timepoints
Trains on all (channel,timepoint) pairs simultaneously instead of individually
"""
...
class BatchMultiRidge:
    ...
    def fit(self, X: torch.Tensor, Y: torch.Tensor, cv_folds: int = 5) -> 'BatchMultiRidge':
        """
        ...
        Returns:
            self: Fitted BatchMultiRidge instance
        """
        X, Y = X.to(self.device), Y.to(self.device)
        n_samples, n_features = X.shape
        n_targets = Y.shape[1]
        
        print(f"BatchMultiRidge: Training on {n_samples} samples, {n_features} features, {n_targets} targets")
        print(f"BatchMultiRidge: Testing {len(self.alphas)} alpha values with {cv_folds}-fold CV")
        ...
        print(f"BatchMultiRidge: Selected alphas range from {self.best_alphas_.min():.2e} to {self.best_alphas_.max():.2e}")
...
def batch_encoding(
    ...
    """
    Batch encoding that trains on all EEG (channel,timepoint) pairs simultaneously
    ...
    print(f"Batch encoding using device: {device}")
    ...
    # Fit BatchMultiRidge model
    model = BatchMultiRidge(alphas=alpha_range, device=device)
    ...
    print(f"Batch encoding completed. Performance range: {scores.min():.3f} to {scores.max():.3f}")
    
    return predictions, best_alphas_reshaped, scores


def batch_fusion_encoding(
    ...
    """
    Batch fusion encoding with convex combination optimization
    ...
    print(f"Batch fusion encoding using device: {device}")
    ...
    for weight in weight_list:
        ...
        # Use BatchMultiRidge for this weight
        model = BatchMultiRidge(alphas=alpha_range, device=device)
    ...
    print(f"Batch fusion encoding completed. Performance range: {scores_np.min():.3f} to {scores_np.max():.3f}")
